...
public class GuiDepartureSwitch extends GuiScreen {
    ...
    private GuiTextField key, search, rotationField, offsetXField, offsetYField, offsetZField;
    ...
    @Override public void initGui() {
        ...
        String offsetYText = offsetYField == null ? offsetText(tile.getOffsetY()) : offsetYField.getText();
        String offsetZText = offsetZField == null ? offsetText(tile.getOffsetZ()) : offsetZField.getText();
        ...
        int offsetWidth = Math.max(24, (rightWidth - 8) / 3);
        offsetXField = numberField(right, 102, offsetWidth, offsetXText);
        offsetYField = numberField(right + offsetWidth + 4, 102, offsetWidth, offsetYText);
        offsetZField = numberField(right + (offsetWidth + 4) * 2, 102, offsetWidth, offsetZText);
        buttonList.add(new GuiButton(3, right, 127, rightWidth, 20, previewText()));
        filter();
    }
    private GuiTextField numberField(int x, int y, int width, String value) {
    ...
    private String offsetText(float value) { return Float.toString(value); }
    private float offset(GuiTextField field) {
        float value = Float.parseFloat(field.getText().trim());
        if (!TileEntityDepartureSwitch.validOffset(value)) throw new NumberFormatException("Offset out of range");
        return value;
    }
    private void resetOffsetFields() {
        offsetXField.setText(offsetText(tile.getOffsetX()));
        offsetYField.setText(offsetText(tile.getOffsetY()));
        offsetZField.setText(offsetText(tile.getOffsetZ()));
    }
    ...
    @Override protected void actionPerformed(GuiButton button) {
        if (button.id == 0 && SwitchModelRegistry.get(selected) != null) {
            ...
            try {
                yaw = SwitchYaw.parse(rotationField.getText());
                offsetX = offset(offsetXField); offsetY = offset(offsetYField); offsetZ = offset(offsetZField);
            }
    ...
    @Override public void drawScreen(int mx, int my, float partial) {
        ...
        int offsetWidth = Math.max(24, (rightWidth - 8) / 3);
        drawString(fontRendererObj, I18n.format("gui.sam.switch.offset"), right, 91, 0xAAAAAA);
        offsetXField.drawTextBox(); offsetYField.drawTextBox(); offsetZField.drawTextBox();
        ...
        if (model == null) {
            fontRendererObj.drawSplitString(I18n.format("gui.sam.switch.missing") + ": " + selected, right, 154, rightWidth, 0xFF7777);
        } else {
            renderPreview(model);
    ...
    private void renderPreview(SwitchModelDefinition model) {
        ...
        if (mesh == null) {
            fontRendererObj.drawSplitString(I18n.format("gui.sam.switch.model_error"), right, 154, rightWidth, 0xFF7777);
            return;
        }
        ...
        int previewHeight = Math.max(20, bottom - 174);
        ...
        try {
            ...
            GL11.glTranslated(right + rightWidth / 2.0, 154 + previewHeight / 2.0, 100);
    ...
    @Override protected void mouseClicked(int x, int y, int button) {
        ...
        offsetXField.mouseClicked(x, y, button); offsetYField.mouseClicked(x, y, button); offsetZField.mouseClicked(x, y, button);
    ...
    @Override protected void keyTyped(char c, int code) {
        ...
        for (GuiTextField field : new GuiTextField[]{offsetXField, offsetYField, offsetZField}) {
            if (field.isFocused()) {
                if (code == Keyboard.KEY_ESCAPE) super.keyTyped(c, code);
                else field.textboxKeyTyped(c, code);
                return;
            }
        }
    ...
    @Override public void updateScreen() {
        key.updateCursorCounter(); search.updateCursorCounter(); rotationField.updateCursorCounter();
        offsetXField.updateCursorCounter(); offsetYField.updateCursorCounter(); offsetZField.updateCursorCounter();
    }
