package jp.me1han.sam.render;

import jp.me1han.sam.client.SwitchMeshRenderer;
import jp.me1han.sam.switchmodel.SwitchModelDefinition;
import jp.me1han.sam.switchmodel.SwitchModelRegistry;
import net.minecraft.client.renderer.tileentity.TileEntitySpecialRenderer;
import net.minecraft.item.ItemStack;
import net.minecraft.tileentity.TileEntity;
import net.minecraftforge.client.IItemRenderer;
import org.lwjgl.opengl.GL11;

public class RendererDepartureSwitch extends TileEntitySpecialRenderer implements IItemRenderer {
    @Override public void renderTileEntityAt(TileEntity tile, double x, double y, double z, float partial) {
        TileEntityDepartureSwitch button = (TileEntityDepartureSwitch) tile;
        SwitchModelDefinition definition = SwitchModelRegistry.getOrDefault(button.modelName);
        GL11.glPushMatrix();
        try {
            GL11.glTranslated(x + 0.5, y, z + 0.5);
            GL11.glRotatef(button.getRotationYaw(), 0, 1, 0);
            if (!SwitchMeshRenderer.INSTANCE.render(definition, button.isActivated(), tile.getWorldObj().getLightBrightnessForSkyBlocks(tile.xCoord, tile.yCoord, tile.zCoord, 0))) {
                SwitchMeshRenderer.INSTANCE.render(SwitchModelRegistry.get(SwitchModelRegistry.DEFAULT_MODEL), false, 0xF000F0);
            }
        } finally { GL11.glPopMatrix(); }
    }
    @Override public boolean handleRenderType(ItemStack stack, ItemRenderType type) { return type != ItemRenderType.FIRST_PERSON_MAP; }
    @Override public boolean shouldUseRenderHelper(ItemRenderType type, ItemStack stack, ItemRendererHelper helper) { return true; }
    @Override public void renderItem(ItemRenderType type, ItemStack stack, Object... data) {
        String name = SwitchModelRegistry.DEFAULT_MODEL;
        if (stack.hasTagCompound() && stack.getTagCompound().hasKey("BlockEntityTag")) {
            String selected = stack.getTagCompound().getCompoundTag("BlockEntityTag").getString("modelName");
            if (!selected.isEmpty()) name = selected;
        }
        GL11.glPushMatrix();
        try {
            if (type == ItemRenderType.EQUIPPED || type == ItemRenderType.EQUIPPED_FIRST_PERSON) GL11.glTranslatef(0.5F, 0.25F, 0.5F);
            else GL11.glTranslatef(0, -0.1F, 0);
            SwitchMeshRenderer.INSTANCE.render(SwitchModelRegistry.getOrDefault(name), false, 0xF000F0);
        } finally { GL11.glPopMatrix(); }
    }
}
