...
- OFF後の待ち時間や戸閉放送中に再びONにしても、進行中の待ち時間・戸閉放送は停止、巻き戻し、重複生成されません。そのまま進行させながらメロディをONへ戻します。
- `tachikawa(false)`などでメロディが停止済みなら先頭から再開します。`tachikawa(true)`で現在のコーラスがまだ鳴っている場合は二重再生や先頭からの再スタートをせず、そのコーラスを継続します。
- 最初の戸閉処理中にもう一度OFFにしても新しい戸閉放送は作りません。戸閉処理が完了した後もメロディがONならsessionを維持し、その後のOFFで新しい待ち時間と戸閉放送を開始します。
...
啓発放送の「発車メロディ終了後にも再生」は、メロディと戸閉放送の両方が通常終了した後に動作します。「発車後間隔（秒）」には終了から啓発放送を始めるまでの待ち時間を指定でき、既定値は0秒です。停止ブロックなどによる途中停止では、この発車後の放送は開始されません。
