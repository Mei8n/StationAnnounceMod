...
未宣言の場合だけ`UNKNOWN`へ補完します。`getScriptType()`を定義した場合は、有効な整数IDを返す必要があります。例外、`null`、Number以外、非整数、未定義ID、関数以外の宣言はload failureとなり、scriptを登録しません。不正な宣言を`UNKNOWN`へ降格させることはありません。
